class Play extends Phaser.Scene {
    ...
    create() {
        ...
        this.player.body.setSize(playerBodyWidth, playerBodyHeight)
            .setOffset(
                ...
                (this.player.height - playerBodyHeight) / 2 + 12
            );
        ...
        this.restartText = this.add.text(width/2, height/2, 'Press R to Restart', 
    ...
    update() {
        if (this.cursors.R.isDown) {
            if (this.bgm) {
                this.bgm.stop(); // Stop the current BGM instance
            }
            this.restartGame();
        }
    ...
    if (seconds % 10 === 0 && seconds !== 0 && this.lastPlayedTime !== seconds) {
        this.sound.play('timerSound', { volume: 0.25 });
        ...
        this.walls.getChildren().forEach(wall => {
            if (wall.x < -wall.displayWidth * 2) {
                wall.setVelocityX(-Phaser.Math.Between(150, 250));
